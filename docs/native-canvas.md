...
`text_editor_macos.rs`がPaintViewの子NSTextViewを管理する。編集中は元の文書を変更せず、対象文字だけを隠した文書プレビューをGPUへ表示する。各キー入力ではSVGをラスタ化せず、AppKitが入力・カーソル・選択範囲を描画する。枠幅で折り返した視覚行の開始位置をUTF-16オフセットで取り出し、確定後に本文・書式と一括してコアへ適用する。取消・空の本文は元の文書を保持する。文字パネルの操作は編集セッションへ反映し、編集中でなければ選択中の文字を直接変更する。
