...
変更された文字レイヤーの字形処理・文書サイズのラスタ化・GPU転送は引き続き必要。大きな文書や長文の描画スレッド分離・部分更新は後続の最適化とする。
...
レイヤー内の可視オブジェクトがすべて選択され、SVGの実際の描画境界が既存テクスチャ内に収まる場合は、描画済みテクスチャへGPUで平行移動を適用する。ズームとパンは共通の座標変換に従い、作品の外へ出た部分はクリップする。一部選択や、元から作品の外へはみ出す文字は、選択オブジェクトだけを移動した一時SVGを描画する。未選択レイヤーのテクスチャと描画順は維持する。
