...
macOSのネイティブキャンバスでは、ペン（P）、長方形／楕円（U／Shift+U）を文書座標の`VectorObject`として作成する。編集可能なベクターレイヤーがなければ自動作成し、描画色を塗りまたは線へ適用する。ベクター選択（V）は制御点メタデータから前面のオブジェクトをヒットテストし、Shiftで複数選択、ドラッグで一括移動できる。選択境界と制御点は作品SVGと分離したGPUオーバーレイで表示し、制御点ドラッグでパス・長方形・楕円を直接再構築する。

ベクター選択の操作メニューから、同じ編集可能レイヤー上の2図形に合体・前面との差分・交差・排他的和を適用できる。塗り色が同じで線のない図形に限定し、結果の外観が曖昧になる組合せを拒否する。差分の左辺は背面、右辺は前面。Skiaで変換を文書座標へ焼き込んだ結果をポータブルなSVGパスとして保存し、1回のUndo/Redoで復元する。複合パスは全体移動できるが、曲線を壊さないため制御点の直接編集は後続工程とする。
